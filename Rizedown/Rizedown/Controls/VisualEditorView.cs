...
public sealed class VisualEditorView : ContentView, IEditorSurface
{
    ...
    {
        // Text change: rizedown://tc/{cursor}/{selection}/{url-safe-base64-text}
        if (e.Url.StartsWith("rizedown://tc/"))
        {
            e.Cancel = true;
            var rest = e.Url["rizedown://tc/".Length..];
        ...
        // Cursor-only update: rizedown://k/{cursor}/{selection}
        else if (e.Url.StartsWith("rizedown://k/"))
        {
            e.Cancel = true;
            var parts = e.Url["rizedown://k/".Length..].Split('/');
            if (parts.Length >= 2)
            {
